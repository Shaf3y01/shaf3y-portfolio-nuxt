...
<template>
  ...
  >
    ...
    <div class="relative flex flex-col items-center">
      ...
      <div
        class="projects-preview-card pointer-events-none absolute top-6 scale-95 opacity-30 blur-[2px]
      ...
      >
        <div
          class="aspect-video w-full border-b border-white/10 bg-eerie/60 overflow-hidden"
          role="presentation"
        >
          <!-- ✅ dynamic preview image -->
          <NuxtImg
            :src="nextPreview.img"
            :alt="`${nextPreview.name} project screenshot`"
            width="800"
            height="450"
            loading="lazy"
            decoding="async"
      ...
      <Transition :name="direction === 'next' ? 'swipe-next' : 'swipe-prev'" mode="out-in">
        ...
        >
          ...
          <!-- text content -->
          <div
            class="projects-card-body p-6 flex flex-col flex-1 text-center md:text-left
                   min-h-[16rem] md:min-h-[14rem] lg:min-h-[13rem]"
            aria-live="polite"
          >
            ...
            <!-- tech stack chips -->
            <ul
              class="projects-stack flex flex-wrap justify-center md:justify-start gap-2 mt-6 pt-1 pb-1
                     max-h-[4.5rem] overflow-hidden"
              :aria-label="`${activeProject.name} technology stack`"
            >
              <li
                v-for="tech in activeProject.stack"
                :key="tech"
                class="projects-chip font-mono text-[0.7rem] leading-none px-2.5 py-1 rounded-full border
      ...
      <div class="mt-8 flex items-center gap-4 text-seasalt/60">
        <button
          class="projects-control flex items-center gap-2 px-4 py-2 rounded-[var(--radius-xl)]
          ...
          type="button"
          @click.stop.prevent="prevProject"
      ...
      <!-- index / counter -->
      <div
        class="projects-counter mt-4 text-[0.75rem] text-seasalt/40 tracking-wide uppercase"
        aria-live="polite"
      >
...
<style scoped>
/* your existing CSS – unchanged, just kept as-is */
:root {
  --color-eerie: #222823;
